...
def test_psa4():
    ...
    try:
        similarity = rc.movie_dict[1196].get_similarity(
            112852, rc.movie_dict, rc.user_dict
        )
        if round(similarity, 2) == 0.82:
            ...
            print(
                f"  failed.  You returned {similarity}.  similarity should be 0.82 rounded to 2 decimals"
            )
    ...
    try:
        ...
        if round(predicted_rating, 2) == 3.65:
            ...
            print(
                f"  failed.  You returned {predicted_rating} Should have retured 3.65 rounded to 2 decimals"
            )
    except Exception as e:
        print(f"  failed.  Your code raised the exception {e}")
