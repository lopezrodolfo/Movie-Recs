...
def test_psa4():
    ...
    try:
        ...
        if set(range(1, 6)) != set(md):
            raise IncorrectCode(
                f"set of movie ids incorrect.  Should be {set(range(1, 6))}"
            )
        for i in range(1, 6):
    ...
    try:
        ...
        if set(range(1, 6)) != set(ud):
            raise IncorrectCode(
                f"set of user ids incorrect.  Should be {set(range(1, 6))}"
            )
        for i in range(1, 6):
            if set(ud[i]) != set(ratings[i]):
                raise IncorrectCode(
                    f"ratings dictionary for user {i} incorrect.  Should be {ratings[i]}"
                )
            for movie_id in ud[i]:
                if ud[i][movie_id] != ratings[i][movie_id]:
                    raise IncorrectCode(
                        f"rating of movie {movie_id} by user {i} incorrect.  Should be {ratings[i][movie_id]}"
                    )
        print("  passed")
        num_correct += 1
    except IncorrectCode as e:
            ...
            for other_movie_id in md:
                if movie_id != other_movie_id:
                    ...
                    if round(similarity, 2) != similarities[movie_id][other_movie_id]:
                        raise IncorrectCode(
                            f"Similarity between {movie_id} and {other_movie_id} incorrect.  Should be {similarities[movie_id][other_movie_id]}  You computed {similarity}"
                        )
        print("  passed")
        num_correct += 1
    except IncorrectCode as e:
    ...
    try:
        ...
        try:
            ...
            print(
                f"  failed.  Your code did not raise BadInputError when invalid movie id is passed to get_similarity"
            )
            raise Exception
        md[1].get_similarity(2, md, ud)
        if 2 not in md[1].similarities or 1 not in md[2].similarities:
            raise IncorrectCode(
                "get_similarity does not save the similarity in both movie entries in the movie dictionary"
            )
        print("  passed")
        num_correct += 1
    except IncorrectCode as e:
    ...
    try:
        ...
        if round(predicted_rating, 2) == 4.11:
            ...
            print(
                f"  failed.  You returned {predicted_rating} Should have retured 4.11 rounded to 2 decimals"
            )
    except Exception as e:
        print(f"  failed.  Your code raised the exception {e}")
    ...
    try:
        ...
        if round(predicted_rating, 2) == 4:
            ...
            print(
                f"  failed.  You returned {predicted_rating} Should have retured 4 rounded to 2 decimals"
            )
    except Exception as e:
        print(f"  failed.  Your code raised the exception {e}")
    ...
    try:
        ...
        if predicted_ratings == [
            ...
            print("Should be ")
            print(
                "[(2, 'M1', 4.11, 5.0), (2, 'M5', 3.82, 5.0), (3, 'M1', 1.5, 0.0), (3, 'M3', 3.0, 3.0), (3, 'M5', 1.43, 5.0), (4, 'M2', 1.1, 4.0), (5, 'M1', 5.0, 2.0), (5, 'M2', 5.0, 2.0), (5, 'M5', 5.0, 2.0)]"
            )
    except Exception as e:
        print(f"  failed.  Your code raised the exception {e}")
        print("Should be ")
    ...
    num_tested += 1
    print(
        "Testing overall correctness by computing correlation for predictions of test ratings"
    )
    try:
        mr = movie_recommendations.Movie_Recommendations(
            "dummy_movies.csv", "dummy_training_ratings.csv"
